class Solution {
//     final int[] diff = {-1, 0, 1, 0, -1};
//     public List<List<Integer>> pacificAtlantic(int[][] heights) {
//         List<List<Integer>> res = new ArrayList<>();
//         int m = heights.length, n = heights[0].length;
//         int[][] pacific = new int[m][n], atlantic = new int[m][n];
//         for(int i = 0; i < m; i++) {
//             for(int j = 0; j < n; j++) {
//                 if(flowsPacific(i, j, heights, pacific) && flowsAtlantic(i, j, heights, atlantic)) {
//                     res.add(new ArrayList<>(Arrays.asList(i, j)));
//                 }
//             }
//         }
        
//         return res;
//     }
    
//     private boolean flowsPacific(int r, int c, int[][] heights, int[][] pacific) {
        
//         if(r == 0 || c == 0) {
//             pacific[r][c] = 1;
//             return true;
//         }
        
//         if(pacific[r][c] == 1) return true;
        
//         boolean toReturn = false;
        
//         int currH = heights[r][c];
//         heights[r][c] = -1;
        
//         for(int i = 0; i < 4; i++) {
//             int nextR = r + diff[i], nextC = diff[i + 1];
            
//             if(nextR < 0 || nextR == heights.length || nextC < 0 || nextC == heights[0].length || 
//                heights[nextR][nextC] == -1) continue;
            
//             if(currH >= heights[nextR][nextC] && flowsPacific(nextR, nextC, heights, pacific)) {
//                 pacific[r][c] = 1;
//                 toReturn = true;
//                 break;
//             }
//         }
//         heights[r][c] = currH;
//         return toReturn;
//     }
    
//     private boolean flowsAtlantic(int r, int c, int[][] heights, int[][] atlantic) {
        
//         if(r == heights.length - 1 || c == heights[0].length - 1) {
//             atlantic[r][c] = 1;
//             return true;
//         }
        
//         if(atlantic[r][c] == 1) return true;
        
//         boolean toReturn = false;
        
//         int currH = heights[r][c];
//         heights[r][c] = -1;
        
//         for(int i = 0; i < 4; i++) {
//             int nextR = r + diff[i], nextC = diff[i + 1];
            
//             if(nextR < 0 || nextR == heights.length || nextC < 0 || nextC == heights[0].length || 
//                heights[nextR][nextC] == -1) continue;
            
//             if(currH >= heights[nextR][nextC] && flowsPacific(nextR, nextC, heights, atlantic)) {
//                 atlantic[r][c] = 1;
//                 toReturn = true;
//                 break;
//             }
//         }
//         heights[r][c] = currH;
//         return toReturn;
//     }
    
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        int rows = heights.length, cols = heights[0].length;
        boolean[][] pac = new boolean[rows][cols];
        boolean[][] atl = new boolean[rows][cols];
        
        for (int col = 0; col< cols; col++){
            dfs(0, col, rows, cols, pac, heights[0][col], heights);
            dfs(rows-1, col,rows, cols, atl, heights[rows-1][col], heights);
        }
        for (int row = 0; row<rows; row++){
            dfs(row, 0,rows, cols, pac, heights[row][0], heights);
            dfs(row, cols-1,rows, cols, atl, heights[row][cols-1], heights);
        }
        List<List<Integer>> result = new ArrayList<List<Integer>>();
        for (int i = 0; i < rows; i++)
            for (int j = 0; j < cols; j++){
                if (pac[i][j] && atl[i][j])
                    result.add(Arrays.asList(i,j));
            }
        return result;
    }
    private void dfs(int row, int col, int rows, int cols, boolean[][] visited, int prevHeight, int[][] heights){
        if (row < 0 || row >= rows || col < 0 || col >= cols || visited[row][col] || prevHeight > heights[row][col])
            return;
        visited[row][col]= true;
        dfs(row+1, col, rows, cols, visited, heights[row][col], heights);
        dfs(row-1, col, rows, cols, visited, heights[row][col], heights);
        dfs(row, col+1, rows, cols, visited, heights[row][col], heights);
        dfs(row, col-1, rows, cols, visited, heights[row][col], heights);
        
    }
}